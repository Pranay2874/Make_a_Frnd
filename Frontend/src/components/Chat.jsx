...
const Chat = () => {
    ...
    const sendMessage = () => {
        if (newMessage.trim() && isConnected) {
            ...
            // Emit to server (usertargetId is the partner's socket ID)
            socket.emit("sendMessage", { partnerId: usertargetId, message: messageText });
            
            // Add own message to the chat (assuming 'username' is in localStorage)
            ...
            <div className="p-4 bg-white border-t border-gray-200">
                <div className="flex space-x-3">
                    <input
                        ...
                        onKeyPress={(e) => { // Allows sending with Enter key
                            if (e.key === 'Enter') {
                                sendMessage();
                            }
                        }}
